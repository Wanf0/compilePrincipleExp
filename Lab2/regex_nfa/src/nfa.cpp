...
void NFA::export_png(const std::string& filename) const {
    ...
    if(ret != 0) {
        ...
        std::cout << "NFA visualization saved as " << filename << ".png" << std::endl;
    }
}
...
std::string NFA::debug_dump() const {
    ...
    for (auto a : accepts_set) oss << "S" << a << " ";
    oss << "\nTransitions:\n";
    std::vector<int> all;
    for (const auto &kv : trans) {
        all.push_back(kv.first);
        for (auto &t : kv.second) all.push_back(t.dest);
    }
...
void NFA::epsilon_closure(const std::unordered_set<int>& src, std::unordered_set<int>& out) const {
    std::queue<int> q;
    for (int s : src) {
        out.insert(s);
        q.push(s);
...
void NFA::move(const std::unordered_set<int>& src, char symbol, std::unordered_set<int>& out) const {
    for (int s : src) {
        auto it = trans.find(s);
        if (it == trans.end()) continue;
        for (auto &t : it->second) {
