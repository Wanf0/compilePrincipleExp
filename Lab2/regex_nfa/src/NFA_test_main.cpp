...
void visualize_nfa(const NFA &nfa, const std::string &filename_base) {
  ...
  std::string cmd =
      "dot -Tpng " + dot_filename + " -o " + filename_base + ".png";
  int ret = std::system(cmd.c_str());
  if (ret != 0) {
    std::cerr << "Graphviz dot command failed: " << cmd << std::endl;
  } else {
    std::cout << "Visualization generated: " << filename_base << ".png"
              << std::endl;
  }
}
...
int main() {
  ...
  std::cout << "Enter a regex (extended operators supported: +, *, ?, |, "
               "parentheses):\n";
