...
void test_string(const NFA &nfa, const DFA &dfa, const std::string &str) {
    ...
    std::cout << "String \"" << str << "\": NFA=" << (accept_nfa ? "ACCEPT" : "REJECT")
...
void interactive_mode() {
    ...
    while (true) {
        std::cout << "regex> ";
        std::string regex;
        if (!std::getline(std::cin, regex) || regex.empty()) break;
...
void batch_mode_from_file(const std::string &path) {
    ...
    if (!ifs) { std::cerr << "Cannot open file: " << path << "\n"; return; }
